...
include("DB.inc.php");
session_start();
$upit1 = "SELECT * FROM pitanja WHERE anketa_test='".$_SESSION["test"]."'";
...
$rezultat2 = mysqli_query($konekcija, $upit2)
	or die("Greska u upitu: " .mysqli_errno($konekcija));
$upit = "SELECT * FROM snimi WHERE anketa_test='".$_SESSION["test"]."' AND korisnik ='".$_SESSION["username"]."'";
$rezultat = mysqli_query($konekcija, $upit)
	or die("Greska u upitu: " .mysqli_errno($konekcija));
if (mysqli_num_rows($rezultat)>0) {
	$upit = "DELETE FROM snimi WHERE anketa_test='".$_SESSION["test"]."' AND korisnik ='".$_SESSION["username"]."'"; 
	$rezultat = mysqli_query($konekcija, $upit)
		or die("Greska u upitu: " .mysqli_errno($konekcija));
}
for ($br = 0; $br<$brojPitanja; $br++) { //brojac za $_SESSION["pitanja"][brojac] i $_SESSION["odgovori"][brojac] ide do broja pitanja u anketi
	...
	VALUES ('".$_SESSION["pitanja"][$br]."', '".$_SESSION["odgovori"][$br]."', '".$_SESSION["test"]."', '".$_SESSION["username"]."')";
	$rezultat3 = mysqli_query($konekcija, $upit3)
		or die("Greska u upitu: " .mysqli_errno($konekcija));
}
$upit = "SELECT idTesta FROM testovi WHERE naziv = '".$_SESSION["test"]."'";
$rezultat = mysqli_query($konekcija, $upit)
	or die("Greska u upitu: " .mysqli_errno($konekcija));
$niz = mysqli_fetch_array($rezultat);
header("Location: popuniTest.php?id=".$niz['idTesta']);
?>